...
/*
==============
idSessionLocal::StartStats
==============
*/
void idSessionLocal::StartStats(const char * nextMap) 
{
	if (!idStr::Icmp("maps/startmap", session->GetCurrentMapName() ))	//don't want to show stats for the start map
	{
		MoveToNewMap(nextMap);
		return;
	}

	if (guiActive == guiEndLevelStats) 
	{
		return;
	}
	...
	//doom2 is always "mapXX"
	if (m_nextMap.Find("map")>0)
	{
		DRLevelID += 1000;
		
		//strip "map" to get the number
		auto mapStr = m_nextMap;
		mapStr.StripLeading("map");
		int levelNum = atoi(mapStr);
		DRLevelID += levelNum;
	}
	else
	{
		//Doom 1 is "eXmX"
		auto mapStr = m_nextMap;
		auto map = atoi(&mapStr[3]);
		//auto ep = atoi(&mapStr[1]);
		//DRLevelID += 100 * ep;
		DRLevelID += map;
	}
	...
	auto secretString = "Secrets: " + idStr(secrets) + idStr("/") + idStr(totalSecrets.Num());

	guiEndLevelStats->SetStateString("kills", killString);
	guiEndLevelStats->SetStateString("items", itemString);
	guiEndLevelStats->SetStateString("secrets_found", secretString);

	SetGUI(guiEndLevelStats, NULL);
...
/*
===============
idSessionLocal::ExitStats
===============
*/
void idSessionLocal::ExitStats() 
{
...
void idSessionLocal::DispatchCommand( idUserInterface *gui, const char *menuCommand, bool doIngame ) {
	...
	if ( gui == guiMainMenu || gui == guiEndLevelStats ) {

...
*/
idSessionLocal::idSessionLocal() {
	guiInGame = guiMainMenu = guiIntro = guiEndLevelStats \
...
void idSessionLocal::RunGameTic() {
	...
	if ( ret.sessionCommand[0] ) {
		...
		else if (!idStr::Icmp(args.Argv(0),"stats"))
		{
			//need to show stats screen
			StartStats(args.Argv(1));
		}
		else if (!idStr::Icmp(args.Argv(0), "statsOver"))
		{
			//need to show stats screen
			ExitStats();
		}
...
void idSessionLocal::Init() {
	...
	guiEndLevelStats = uiManager->FindGui("guis/drm_stats.gui", true, false, true);

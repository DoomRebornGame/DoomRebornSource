...
/*
==============
idInventory::SetMaxAmmoIndexForAmmoClass
==============
*/
void idInventory::SetMaxAmmoForAmmoClass(idPlayer *owner, const char *ammo_classname, int max_ammo)
{
	owner->spawnArgs.SetInt(va("max_%s", ammo_classname), max_ammo);
}
...
bool idInventory::Give( idPlayer *owner, const idDict &spawnArgs, const char *statname, const char *value, int *idealWeapon, bool updateHud ) {
	...
	else if (!idStr::Icmp(statname,"carry_backpack"))
	{
		//magic numbers, I know...
		owner->inventory.SetMaxAmmoForAmmoClass(owner, "ammo_bullets", 400);
		owner->inventory.SetMaxAmmoForAmmoClass(owner, "ammo_shells", 100);
		owner->inventory.SetMaxAmmoForAmmoClass(owner, "ammo_cells", 600);
		owner->inventory.SetMaxAmmoForAmmoClass(owner, "ammo_rockets", 100);
		ammoPulse = true;
	}
	else {
		// unknown item
...
void idPlayer::UpdateHudAmmo( idUserInterface *_hud ) {
	...
	if ( ammoamount < 0 || !weapon.GetEntity()->IsReady() ) {
		...
		//go through ammo types and update them
		//bullets
		auto bulletIndex = idWeapon::GetAmmoNumForName("ammo_bullets");
		_hud->SetStateString("player_ammo_bullets", va("%i", inventory.ammo[bulletIndex]));
		_hud->SetStateString("player_ammo_max_bullets", va("%i", inventory.MaxAmmoForAmmoClass(this,"ammo_bullets")));

		//shells
		auto shellIndex = idWeapon::GetAmmoNumForName("ammo_shells");
		_hud->SetStateString("player_ammo_shells", va("%i", inventory.ammo[shellIndex]));
		_hud->SetStateString("player_ammo_max_shells", va("%i", inventory.MaxAmmoForAmmoClass(this, "ammo_shells")));


		//rockets
		auto rocketIndex = idWeapon::GetAmmoNumForName("ammo_rockets");
		_hud->SetStateString("player_ammo_rockets", va("%i", inventory.ammo[rocketIndex]));
		_hud->SetStateString("player_ammo_max_rockets", va("%i", inventory.MaxAmmoForAmmoClass(this, "ammo_rockets")));


		//cells
		auto cellIndex = idWeapon::GetAmmoNumForName("ammo_cells");
		_hud->SetStateString("player_ammo_cells", va("%i", inventory.ammo[cellIndex]));
		_hud->SetStateString("player_ammo_max_cells", va("%i", inventory.MaxAmmoForAmmoClass(this, "ammo_cells")));
...
bool idPlayer::Give( const char *statname, const char *value ) {
	...
	if ( !idStr::Icmp( statname, "health" ) ) 
	{
		...
		std::string healthString(value);
		auto divider(healthString.find(";"));
...
void idPlayer::AdjustSpeed( void ) {
	...
	if ( spectating ) {
		...
		bobFrac = 0.0f;
	} else if ( !physicsObj.OnLadder() && ( usercmd.buttons & BUTTON_RUN || GetUserInfo()->GetBool("ui_autoRun") ) && ( usercmd.forwardmove || usercmd.rightmove ) && ( usercmd.upmove >= 0 ) ) {
...
void idPlayer::Damage( idEntity *inflictor, idEntity *attacker, const idVec3 &dir,
	...
	if ( health > 0 ) 
	{
		...
		//JONNY
		//instead let's flash the screen red
		playerView.Flash(colorRed, 125);
	}
